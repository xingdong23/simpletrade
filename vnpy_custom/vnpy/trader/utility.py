...
import pandas as pd
import pandas_ta as ta  # 已替换为pandas-ta
import pandas as pd
import pandas_ta as ta  # 已替换为pandas-ta
import pandas as pd
...
class ArrayManager:
    ...
    def sma(self, n: int, array: bool = False) -> float | np.ndarray:
        ...
        """
        result_array: np.ndarray = ta.sma(_to_series(self.close), length=n).values
        if array:
            return result_array
    ...
    def ema(self, n: int, array: bool = False) -> float | np.ndarray:
        ...
        """
        result_array: np.ndarray = ta.ema(_to_series(self.close), length=n).values
        if array:
            return result_array
    ...
    def kama(self, n: int, array: bool = False) -> float | np.ndarray:
        ...
        KAMA.
        """
        result_array: np.ndarray = ta.kama(_to_series(self.close), length=n).values
        if array:
            return result_array
    ...
    def wma(self, n: int, array: bool = False) -> float | np.ndarray:
        ...
        WMA.
        """
        result_array: np.ndarray = ta.wma(_to_series(self.close), length=n).values
        if array:
            return result_array
    ...
    def apo(
        ...
        APO.
        """
        result_array: np.ndarray = ta.apo(_to_series(self.close), fast=fast_period, slow=slow_period).values
        if array:
            return result_array
    ...
    def cmo(self, n: int, array: bool = False) -> float | np.ndarray:
        ...
        CMO.
        """
        result_array: np.ndarray = ta.cmo(_to_series(self.close), length=n).values
        if array:
            return result_array
    ...
    def cci(self, n: int, array: bool = False) -> float | np.ndarray:
        ...
        """
        result_array: np.ndarray = ta.cci(_to_series(self.high), _to_series(self.low), _to_series(self.close), length=n).values
        if array:
            return result_array
    ...
    def atr(self, n: int, array: bool = False) -> float | np.ndarray:
        ...
        """
        result_array: np.ndarray = ta.atr(_to_series(self.high), _to_series(self.low), _to_series(self.close), length=n).values
        if array:
            return result_array
    ...
    def rsi(self, n: int, array: bool = False) -> float | np.ndarray:
        ...
        """
        result_array: np.ndarray = ta.rsi(_to_series(self.close), length=n).values
        if array:
            return result_array
    ...
    def macd(
        ...
        MACD.
        """
        macd, signal, hist = ta.macd(_to_series(
            self.close), fast=fast_period, slow=slow_period, signal=signal_period
        ).values
        if array:
    ...
    def adx(self, n: int, array: bool = False) -> float | np.ndarray:
        ...
        ADX.
        """
        result_array: np.ndarray = ta.adx(_to_series(self.high), _to_series(self.low), _to_series(self.close), length=n).values
        if array:
            return result_array
    ...
    def boll(
        ...
        Bollinger Channel.
        """
        mid_array: np.ndarray = ta.sma(_to_series(self.close), length=n).values
    ...
    def keltner(
        ...
        """
        mid_array: np.ndarray = ta.sma(_to_series(self.close), length=n).values
        atr_array: np.ndarray = ta.atr(_to_series(self.high), _to_series(self.low), _to_series(self.close), length=n).values
