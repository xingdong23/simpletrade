...
- 🔄 **数据下载/导入功能实现 (方案A)** (进度: 40% - 下载调试中，导入代码完成但未测试，**阻碍：Tiger Gateway 加载失败**)
...
### [核心问题] `vnpy_tiger` 加载失败
- **问题描述**: 尽管执行了 `pip install -e .` 使得 `vnpy_tiger` 在 Python 解释器中可直接导入，并且依赖 `tigeropen` 已确认安装，应用启动时 `simpletrade/main.py` 中的 `import vnpy_tiger` 可能仍然失败，导致 Tiger Gateway 未注册。
- **尝试的解决方案**: `pip install -e .`, `pip install tigeropen`, 添加详细日志输出，创建测试脚本验证导入功能。
- **状态**: **进行中 (部分进展)**
- **下一步**: 添加更详细的日志输出，考虑在代码中显式添加 vnpy_tiger 到 sys.path，测试其他 Gateway 是否可以正常工作。
...
2.  添加更详细的日志输出，以便更好地定位问题
3.  考虑在代码中显式添加 vnpy_tiger 到 sys.path
4.  测试其他 Gateway 是否可以正常工作
5.  (成功后) 测试数据下载功能 `/api/data/download`。
6.  (成功后) 测试数据导入功能 `/api/data/import`。
7.  (数据可用后) 继续前后端联调。
8.  完善各模块功能、文档和测试。
9.  处理 Pydantic V2 警告。
