...
11. **[新增]** 使用 `pip install -e .` 处理本地自定义包 (`vnpy_tiger`) 的安装（作为特殊情况）。 **[可能不再需要，待确认]**
12. **[新增]** 在必要时使用 `pip` 安装 `conda` 源缺失的依赖 (`tigeropen`)（作为特殊情况）。
13. ~~**[新增]** 使用vendors目录管理第三方依赖，包括vnpy、vnpy_tiger、vnpy_datamanager和vnpy_ib等模块。~~ **[已废弃]**
14. **[重大变更]** 放弃使用 `vnpy` 及其插件的源码集成方式 (Git Submodules in `vendors/`)，**改为使用标准的包管理器 (`pip`/`conda`) 安装**。 (Rationale: 简化环境管理，避免本地安装和路径问题)。
...
- **状态**: **进行中 (部分进展)**
- **下一步**: 确认老虎证券账户的市场权限，尝试使用有权限的市场（如港股或A股）进行测试，或者联系老虎证券客服解决权限问题。 (依赖标准库安装后重新测试)
