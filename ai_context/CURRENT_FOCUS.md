...
12. **项目结构重组**
    ...
    - 相关文件: `tests/`, `examples/`, `docs/`, `vendors/`
    - 成果: 成功重组项目结构，创建了测试、示例、文档和vendors目录，并移除了临时脚本和测试文件
    - 完成时间: 2024-04-12 17:15
...
24. **[新增] 项目目录结构整理**
    - 优先级: 高
    - 状态: **已完成**
    - 描述: 全面整理项目目录结构，移除临时脚本和测试文件，创建了vendors目录管理第三方依赖，将vnpy、vnpy_tiger、vnpy_datamanager和vnpy_ib移动到vendors目录，更新了.gitmodules文件和README.md文件以反映新的目录结构。
    - 相关文件: `.gitmodules`, `README.md`, `vendors/`
    - 完成时间: 2024-04-12 17:15

25. **[新增] 更新脚本和文件以反映新的目录结构**
    - 优先级: 高
    - 状态: **已完成**
    - 描述: 更新了相关脚本和文件以反映新的目录结构，包括docs/startup_guide.md、scripts/setup_vnpy.py、scripts/update_vnpy.py、scripts/download_tiger_data.py和simpletrade/main.py。
    - 相关文件: `docs/startup_guide.md`, `scripts/setup_vnpy.py`, `scripts/update_vnpy.py`, `scripts/download_tiger_data.py`, `simpletrade/main.py`
    - 完成时间: 2024-04-12 17:45

26. **[新增] 添加qlib数据格式导入支持**
    - 优先级: 高
    - 状态: **进行中**
    - 描述: 添加对qlib数据格式的导入支持，以便能够导入更多的数据源。包括读取qlib二进制格式数据，转换为我们系统的格式，并保存到数据库。
    - 相关文件: `simpletrade/apps/st_datamanager/importers/qlib_importer.py`
    - 计划开始时间: 2024-04-12 18:00

27. **[新增] 完善qlib数据格式导入支持的实现**
    - 优先级: 高
    - 状态: **计划中**
    - 描述: 完善qlib数据格式导入支持的实现，包括实现读取qlib二进制格式数据的功能，处理日期和时间格式，转换为BarData格式，并添加错误处理和日志记录。
    - 相关文件: `simpletrade/apps/st_datamanager/importers/qlib_importer.py`
    - 计划开始时间: 2024-04-13
...
20. ✅ **创建启动指南文档** (已完成)
21. ✅ **全面整理项目目录结构** (已完成)
22. ✅ **更新脚本和文件以反映新的目录结构** (已完成)
23. ⏳ **添加qlib数据格式导入支持** (进行中)
24. ⏳ **完善qlib数据格式导入支持的实现** (计划中)
...
5.  **实现qlib数据格式导入支持**: 创建新的导入器模块，支持从 qlib 格式数据中导入数据。
6.  **完善qlib数据格式导入支持的实现**: 实现读取qlib二进制格式数据的功能，处理日期和时间格式，转换为BarData格式。
7.  (数据可用后) 联调数据管理: 连接前端与后端API (概览、下载状态、导入、导出、删除、查看)。
8.  连接仪表盘的市场概览与后端API或WebSocket。
9.  实现交易中心等其他模块与后端API的对接。
10. 编写测试计划文档和单元测试。
11. 添加数据可视化组件。
12. 测试老虎证券Gateway (连接和交易功能)。
13. 编写用户文档。
14. 处理 Pydantic V2 警告。
