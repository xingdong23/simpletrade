...
## 2024-04-17 23:00: talib与vnpy兼容性问题的处理
**决策**: 添加一个简单的测试路由，不依赖于talib，以便进行前后端集成测试。同时将talib与vnpy兼容性问题记录为未解决的问题，待后续解决。
**原因**:
- 在启动后端服务时，遇到talib库相关的符号加载错误：`symbol not found in flat namespace '_TA_ACCBANDS'`
- 系统TA-Lib库（0.6.4）与Python ta-lib包（0.6.3）版本不匹配
- 尝试了多种方法但未能解决该问题
**替代方案**: 继续尝试解决talib与vnpy兼容性问题，或者使用Docker创建隔离环境。
**影响**: 暂时无法使用依赖于talib的功能，如技术分析指标计算。前后端集成测试可以继续进行，但功能受限。

## 2024-04-14 14:00: 开发策略调整: 前端优先
...
## 2024-04-14 10:00: 需求重大调整: 内置数据与简化策略
...
## 2024-04-16 15:00: 清理冗余代码并规范 API 实现路径
...
## [已废弃] 2024-04-16 15:30: 更新 AI 协作指南以强调代码整洁
...
## [已废弃] 2024-04-16 16:00: 确定数据下载/导入实现方案
...
## [部分调整] 2024-04-16 16:30: Tiger Gateway 按需连接
...
## [已废弃] 2024-04-16 17:00: 本地自定义包 (`vnpy_tiger`) 的安装方式
...
## [部分调整] 2024-04-16 17:30: 处理 conda 源缺失的依赖 (`tigeropen`)
...
## 2024-04-13 11:30: vnpy_tiger 特殊处理
...
## 2024-04-17 09:30: 数据中心设计方案：历史数据导入与 Qlib 兼容性
**决策**: SimpleTrade 的数据中心将采用以下策略处理历史数据和 Qlib 兼容性：
    1.  **历史数据来源**: 主要通过外部导入（CSV 文件、本地 Qlib 数据集），可选支持 Gateway 下载作为补充。
    2.  **核心存储**: 使用 PostgreSQL + TimescaleDB 存储规范化的行情数据 (OHLCV 等)。
    3.  **Qlib 兼容性**: 提供一个数据转换层，按需从 PostgreSQL 查询数据并转换为 Qlib 所需的高性能格式，供 AI 策略回测使用。
    4.  **Gateway 角色**: 主要负责实时行情数据获取和交易执行。
    5.  **实现**: 数据中心模块需包含数据导入器 (Importers) 和 Qlib 格式转换器。
**原因**:
-   vnpy Gateway 获取历史数据的能力和效率可能有限。
-   需要支持用户导入已有的或预处理的数据集（如 Qlib 数据）。
-   AI 策略依赖 Qlib 的特定高性能数据格式进行回测。
-   结构化存储 (PostgreSQL) 提供了通用性和灵活性，便于 API 查询和其他非 Qlib 用途。
-   明确了 Gateway 和数据中心在数据处理流程中的不同职责。
**替代方案**:
-   完全依赖 Gateway 下载历史数据。
-   直接在数据库中存储 Qlib 格式（可能导致存储冗余和查询复杂性增加）。
-   要求 Qlib 策略直接读取 PostgreSQL (可能性能不佳)。
**影响**:
-   明确了数据中心的实现重点（导入器、转换器、数据库交互）。
-   确保了对 Qlib AI 策略的数据支持。
-   分离了历史数据管理和实时数据/交易流。
-   需要在数据中心模块中投入开发资源实现导入和转换逻辑。
